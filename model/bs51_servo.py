...
def manual_model(bs_dict, rpy_angles):
    ...
    LUpperLipVert     = map_range(bs_dict['MouthUpperUpLeft'] , 0, 1, 0.76, 0) + map_range(bs_dict['MouthPucker'], 0.4, 1, 0, 0.24) 
    RUpperLipVert    = map_range(bs_dict['MouthUpperUpRight'], 0, 1, 0.76, 0) + map_range(bs_dict['MouthPucker'], 0.4, 1, 0, 0.24) 

    LLowerLipVert   = map_range(bs_dict['MouthLowerDownLeft'] , 0, 0.8, 0, 0.8) + map_range(bs_dict['MouthPucker'], 0, 1, 0, -0.2) 
    RLowerLipVert  = map_range(bs_dict['MouthLowerDownRight'], 0, 0.8, 0, 0.8) + map_range(bs_dict['MouthPucker'], 0, 1, 0, -0.2) 

    LSmile    = map_range((bs_dict['MouthSmileLeft']  + bs_dict['MouthLeft']  ), 0 ,1, 0, 0.77) + map_range(bs_dict['MouthPucker'], 0.1, 1, 0.23, 0) 
    RSmile   = map_range((bs_dict['MouthSmileRight'] + bs_dict['MouthRight'] ), 0 ,1, 0, 0.77) + map_range(bs_dict['MouthPucker'], 0.1, 1, 0.23, 0) 

    LSad  = map_range(bs_dict['MouthStretchLeft']  , 0, 0.5, 0.5 , 0) 
    RSad = map_range(bs_dict['MouthStretchRight'] , 0, 0.5, 0.5 , 0) 
    ...
    mouth = [LUpperLipVert, RUpperLipVert, LLowerLipVert, RLowerLipVert, LSmile, RSmile, LSad, RSad, jawOpenLeft, jawOpenRight, LJawForward, RJawForward]
